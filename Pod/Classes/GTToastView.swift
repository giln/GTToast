...
open class GTToastView: UIView, GTAnimatable {
    ...
    fileprivate lazy var contentSize: CGSize = { [unowned self] in
        CGSize(width: self.frame.size.width - self.config.contentInsets.leftAndRight, height: self.frame.size.height - self.config.contentInsets.topAndBottom)
    }()
